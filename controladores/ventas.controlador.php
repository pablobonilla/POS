	...
	static public function ctrCrearVenta(){
		...
		if(isset($_POST["nuevaVenta"])){
			...
			$datos = array("id_vendedor"=>$_POST["idVendedor"],
						  "id_delivery"=>$_POST["seleccionarDelivery"],
						  "id_mesa"=>$_POST["seleccionarMesa"],
			...
			if($respuesta == "ok"){
				...
				//** Pablo **/
				$impresora = "80mm Series Printer";

				$conector = new WindowsPrintConnector($impresora);
				
				
				$img = EscposImage::load("./extensiones/tcpdf/pdf/images/RetroLogo2.png", false);  //Logo del Negocio		
				...
				$printer -> text("RETRO PIZZA"."\n");//Nombre de la empresa

				$printer -> selectPrintMode(); 

				$printer -> text("RNC: 1-32-17493-3"."\n");//Nit de la empresa

				$printer -> text("Dirección: Avenida Yapur Dumit 200"."\n");//Dirección de la empresa

				$printer -> text("Teléfono: (809) 233-1990"."\n");//Teléfono de la empresa
				...
				$printer -> text("           FACTURA AL  CONTADO               "."\n");
				...
				foreach ($listaProductos as $key => $value) {
					...
					//** 
					$printer->text($value["cantidad"]." X "."$ ".number_format($value["precio"],2)."\n");
					$printer -> text("   ");
					$printer -> text($letreroItbi."\n");

					$printer -> setEmphasis(true);
					//$printer -> setTextSize(8, 1);
					$printer-> text("   ".$value["descripcion"]."\n");//Nombre del producto
					$printer -> setEmphasis(false);

					$printer->setJustification(Printer::JUSTIFY_RIGHT);

					// $printer->text("$ ".number_format($value["precio"],2)." Und x "
					// .$value["cantidad"]." = $ ".number_format($value["total"],2)."\n");
					$printer->text("$ ".number_format($valorItbi,2)."        "); 
					$printer->text("$ ".number_format($value["total"],2)."\n");
					
					// $printer->text($value["cantidad"]." X "."$ ".number_format($value["precio"],2)." = $ ".number_format($value["total"],2)."\n");
				...
				$printer->setJustification(Printer::JUSTIFY_RIGHT);
				$printer->text("$ ".number_format($_POST["nuevoPrecioImpuesto"],2)."\n"); //ahora va el impuesto
